...
public class ProductReviewAnalyzer {
    ...
    public TreeMap<String, Long> buildCategoryFrequencyMap() {
        ...
        return categories.stream().sorted().collect(Collectors.groupingBy(String::toLowerCase, TreeMap::new, Collectors.counting()));
    }
    ...
    public List<String> getTopNCategories(int n) {
        //TODO - implement this method
        return buildCategoryFrequencyMap().entrySet().stream().sorted(Map.Entry.comparingByValue()).map(Map.Entry::getKey).limit(n).toList();
    }
    ...
    public List<String> getCategoriesStartingWith(char prefix) {
        //TODO - implement this method
        return categories.stream().filter(s -> s.charAt(0)==prefix).toList();
    }
    ...
    public Optional<String> getMostReviewedInRange(String from, String to) {
        //TODO - implement this method
       return buildCategoryFrequencyMap().subMap(from, true, to, true).entrySet().stream().map(Map.Entry::getKey).findFirst();
    }
}
